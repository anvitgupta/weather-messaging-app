...
;; complete specification.
;;
(defn args [msg] (-> msg str words rest))
...
;; complete specification.
;;
(defn format-hour [h] (if (= h 12)(str "12pm")(if (> h 12)(str (mod h 12) "pm")(if (= h 0)(str "12am")(str h "am")))))
...
;; complete specification.
;;
(defn formatted-hours [hours] (str "from " (format-hour (get hours :start)) " to " (format-hour (get hours :end)) " in " (get hours :location)))
...
(defn office-hours [{:keys [args cmd]}]
  (if (= nil (instructor-hours (nth (vec args) 0))) (str "there are no office hours on that day")
    (formatted-hours (instructor-hours (nth (vec args) 0)))))
...
(def routes {"default"  (stateless (fn [& args] "Unknown command."))
             ...
             "What's the weather in" forecast/get-forecast
             "expert" add-expert
             "ask" ask-experts
